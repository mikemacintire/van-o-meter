...
**Stale column (added 2026-09-08):** `stale` is 1 when the quota payload was byte-identical to the previous tick's — the cloud replaying a cached snapshot for a unit that has dropped off WiFi (A froze for 44 min, 4.0 h, 4.2 h, 81 min and 74 min on the night of 2026-09-07 while B updated every 30 s). `history.load_rows` derives the same verdict for older rows from the 19 logged columns, and in both cases a reading only counts as stale after `STALE_AFTER_S` (300 s, the Overview badge's threshold) of sitting unchanged. Stale rows never feed a watts mean or a SOC endpoint; a bucket with no fresh row is emitted as `None`, and `history._bridge` fills its power metrics with the counter rate between the bounding fresh rows (flagged in the series' `bridged` list). The History chart draws those dashed, draws nothing where nothing bridged, and joins SOC across a gap with a dotted straight line. Known limit: the cloud can serve a payload whose BMS block is fresh while the `pd.*` block (watts + counters) is still frozen, so a bridge after such a reconnect leans late — total energy stays right, timing does not. Counter-based energy totals were never affected.
...
**Hardware discharge floor on B (policy since 2026-08-25):** the balancer is cloud-only, so anything that severs the machine from the cloud mid-transfer (PC WiFi outage 2026-08-25: 4.5 h, B drained 80→6%; write-bounce flap 2026-08-16: B to 13.6%) leaves B's relay on with no software able to cut it. The backstop is B's own `min_dsg_soc` (Control view "Discharge Floor"), which should sit at **15%** = `emergency_b_floor`. It's currently **5** — a floor of 15 wouldn't take with B at 6% (suspected firmware cap at current SOC, see docs/api.md); raise it once B recharges above 15%.
